    ...
    switch (n.kind) {
        ...
        case NodeKind::ConstPy: {
            ...
            if (n.symbol != 0xFFFF'FFFF && n.symbol != 0) {
                i.imm = static_cast<std::uint16_t>(symbol_const(n.symbol));
                ...
                i.imm = static_cast<std::uint16_t>(add_constant(Value::object(reinterpret_cast<PyObj*>(s))));
            }
            ins(b->body, i);
            return;
        }
...
Result<void> Scheduler::run() noexcept {
    ...
    for (NodeId leader : order) {
        ...
        for (const Instr& i : b->body) {
            ...
            unit_.code.push_back(i);  // copy the full 8-byte Instr
    ...
    for (NodeId leader : order) {
        ...
        for (auto& [site, target] : b->term_fixups) {
            ...
            if (tb && at < unit_.code.size()) {
                unit_.code[at].imm = static_cast<std::uint16_t>(tb->start_pc);
            }
        }
    }
